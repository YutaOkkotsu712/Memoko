...
const stroke = (d, color, w) => `<path d="${d}" fill="none" stroke="${color}" stroke-width="${w}" stroke-linecap="square" stroke-linejoin="miter"/>`;
const px = (x, y, w, h, fill, cls = '', opacity) => `<rect${cls ? ` class="${cls}"` : ''} x="${x}" y="${y}" width="${w}" height="${h}" fill="${fill}"${opacity === undefined ? '' : ` opacity="${opacity}"`}/>`;
/** Little ghost/soul blob centered on local (0,0); position via a wrapping <g>. */
const wisp = (cls) => `<path class="${cls}" d="M0 -2.6 q2.6 0 2.6 2.6 q0 2.1 -1.25 3.5 q-0.65 -0.9 -1.35 -0.9 t-1.35 0.9 Q-2.6 2.1 -2.6 0 q0 -2.6 2.6 -2.6 Z" fill="${WISP}" opacity="0.9"/>`;
function hairShell(hair) {
...
function pixelEyes(kind) {
    if (kind === 'alert') {
        return (`<g class="mk-blink"><g class="mk-eyes">` +
            px(9, 17, 4, 5, INK) +
            px(19, 17, 4, 5, INK) +
            px(10, 17, 1, 1, '#fff') +
            px(20, 17, 1, 1, '#fff') +
...
function features(pose) {
    switch (pose) {
        ...
        case 'laptop':
            ...
            return (pixelEyes('alert') +
                px(15, 23, 2, 2, MOUTH) +
                px(8, 22, 3, 2, BLUSH, '', 0.5) +
                px(21, 22, 3, 2, BLUSH, '', 0.5));
        case 'hurt':
            return (stroke('M10 20 L13.8 18.7', INK, 1.35) +
...
function head(pose) {
    const facePose = pose === 'sit' || pose === 'wave' ? 'fresh' :
        pose === 'book' ? 'book' :
            pose;
...
export function faceSvg(state, size) {
    const extras = state === 'critical'
...
export function spriteSvg(pose, size) {
    switch (pose) {
        case 'critical':
...
const suit = (pale = false) => pale ? DRESS_PALE : DRESS;
const limb = (pale = false) => pale ? LIMB_PALE : LIMB;
function boot(x, y) {
    return px(x, y, 7, 3, SHOE) + px(x + 1, y - 1, 4, 1, SHOE);
}
function standingLegs(pale = false) {
    return (`<g class="sp-legA">${stroke('M21.4 32.5 L21.4 44.2', limb(pale), 2.5)}${boot(19, 44)}</g>` +
        `<g class="sp-legB">${stroke('M26.6 32.5 L26.6 44.2', limb(pale), 2.5)}${boot(25, 44)}</g>`);
}
function arm(cls, d, pale = false, handX = 0, handY = 0) {
    return `<g class="${cls}">${stroke(d, limb(pale), 2.8)}${px(handX, handY, 3, 3, limb(pale))}</g>`;
}
function dress(y, pale = false, seated = false) {
    const cloth = suit(pale);
    const skirtTop = y + (seated ? 5 : 6);
    const skirtBottom = y + (seated ? 13 : 14);
    return (px(20, y, 8, 6, cloth) +
...
function standSvg(pose, size) {
    ...
    return (`<svg viewBox="0 0 48 50" width="${w}" height="${size}" class="sp ${cls}" aria-hidden="true" shape-rendering="crispEdges" style="display:block;overflow:visible">` +
...
function watchSvg(size) {
    const w = Math.round((size * 48) / 50);
    return (`<svg viewBox="0 0 48 50" width="${w}" height="${size}" class="sp sp-watch" aria-hidden="true" shape-rendering="crispEdges" style="display:block;overflow:visible">` +
...
function cheerSvg(size) {
    const w = Math.round((size * 48) / 50);
    return (`<svg viewBox="0 0 48 50" width="${w}" height="${size}" class="sp sp-cheer" aria-hidden="true" shape-rendering="crispEdges" style="display:block;overflow:visible">` +
...
function hurtSvg(size) {
    const w = Math.round((size * 48) / 50);
    return (`<svg viewBox="0 0 48 50" width="${w}" height="${size}" class="sp sp-hurt" aria-hidden="true" shape-rendering="crispEdges" style="display:block;overflow:visible">` +
        `<g class="sp-flinch">` +
...
function lyingSvg(size) {
    ...
    return (`<svg viewBox="0 0 64 50" width="${w}" height="${size}" class="sp sp-lying" aria-hidden="true" shape-rendering="crispEdges" style="display:block;overflow:visible">` +
...
function dangleLegs() {
    return (`<g class="sp-dangleA">${stroke('M21.6 33.6 L21.6 45', LIMB, 2.5)}${boot(18, 45)}</g>` +
...
function bookLegs() {
    return (`<g class="sp-dangleA">${stroke('M22.4 33.8 L22 44.2', LIMB, 2.5)}${boot(18, 44)}</g>` +
...
function sitSvg(size) {
    const w = Math.round((size * 48) / 50);
    return (`<svg viewBox="0 0 48 50" width="${w}" height="${size}" class="sp sp-sit" aria-hidden="true" shape-rendering="crispEdges" style="display:block;overflow:visible">` +
        `<g class="sp-sitbob">` +
        dangleLegs() +
        SEATED_DRESS +
...
function laptopSvg(size) {
    const w = Math.round((size * 48) / 50);
    return (`<svg viewBox="0 0 48 50" width="${w}" height="${size}" class="sp sp-laptop" aria-hidden="true" shape-rendering="crispEdges" style="display:block;overflow:visible">` +
        `<g class="sp-sitbob">` +
        dangleLegs() +
        SEATED_DRESS +
...
function bookSvg(size) {
    const w = Math.round((size * 48) / 50);
    return (`<svg viewBox="0 0 48 50" width="${w}" height="${size}" class="sp sp-book" aria-hidden="true" shape-rendering="crispEdges" style="display:block;overflow:visible">` +
        `<g class="sp-bookbob">` +
        bookLegs() +
        SEATED_DRESS +
...
function napSvg(size) {
    const w = Math.round((size * 48) / 50);
    return (`<svg viewBox="0 0 48 50" width="${w}" height="${size}" class="sp sp-nap" aria-hidden="true" shape-rendering="crispEdges" style="display:block;overflow:visible">` +
...
function waveSvg(size) {
    const w = Math.round((size * 48) / 50);
    return (`<svg viewBox="0 0 48 50" width="${w}" height="${size}" class="sp sp-wave" aria-hidden="true" shape-rendering="crispEdges" style="display:block;overflow:visible">` +
...
function yawnSvg(size) {
    const w = Math.round((size * 48) / 50);
    return (`<svg viewBox="0 0 48 50" width="${w}" height="${size}" class="sp sp-yawn" aria-hidden="true" shape-rendering="crispEdges" style="display:block;overflow:visible">` +
        `<g class="sp-yawnbob">` +
        dangleLegs() +
        SEATED_DRESS +
...
function doodleSvg(size) {
    const w = Math.round((size * 48) / 50);
    return (`<svg viewBox="0 0 48 50" width="${w}" height="${size}" class="sp sp-doodle" aria-hidden="true" shape-rendering="crispEdges" style="display:block;overflow:visible">` +
