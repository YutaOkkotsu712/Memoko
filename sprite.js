// Memoko sprite art — JS port of src/content/ui/avatar.ts, plus three new
// idle poses (sit · laptop · nap) and a sleeping face. Pure inline SVG, no
// assets. Colors route through --mk-* CSS vars so tint syncs with the theme.
// Exposes window.Memoko = { faceSvg, spriteSvg, POSES, STATUS }.
(function () {
  'use strict';

  var HAIR = 'var(--mk-hair, #e893a8)';
  var INK = 'var(--mk-ink, #503a44)';
  var SKIN = 'var(--mk-skin, #ffd9c4)';
  var SKIN_PALE = 'var(--mk-skin-pale, #efdfd6)';
  var LIMB = 'var(--mk-limb, #f2bd97)';
  var LIMB_PALE = 'var(--mk-limb-pale, #ddc4b4)';
  var DRESS = 'var(--mk-dress, #6f86ad)';
  var DRESS_PALE = 'var(--mk-dress-pale, #8a93a6)';
  var SHOE = 'var(--mk-shoe, #56404c)';
  var MOUTH = 'var(--mk-mouth, #cf6a78)';
  var BLUSH = 'var(--mk-blush, #ff9eb0)';
  var SWEAT = 'var(--mk-sweat, #8fd0f5)';
  var WISP = 'var(--mk-wisp, #aac8e4)';
  var SPARK = 'var(--mk-spark, #ffcf6e)';
  // Laptop palette (kept local — only this prop uses it).
  var LID = 'var(--mk-lid, #43506a)';
  var DECK = 'var(--mk-deck, #b7c2d6)';
  var SCREEN = 'var(--mk-screen, #cdeaff)';
  ...
  function wisp(cls) {
    return '<path class="' + cls + '" d="M0 -2.6 q2.6 0 2.6 2.6 q0 2.1 -1.25 3.5 q-0.65 -0.9 -1.35 -0.9 t-1.35 0.9 Q-2.6 2.1 -2.6 0 q0 -2.6 2.6 -2.6 Z" fill="' + WISP + '" opacity="0.9"/>';
  }

  function features(pose) {
    switch (pose) {
      case 'fresh':
      case 'cheer':
        return (
          ...
          '<g fill="' + BLUSH + '" opacity="0.8"><ellipse cx="9.9" cy="22.3" rx="2" ry="1.15"/><ellipse cx="22.1" cy="22.3" rx="2" ry="1.15"/></g>'
        );
      case 'healthy':
        return (
          '<g class="mk-blink"><ellipse cx="12" cy="19.6" rx="1.8" ry="2.4" fill="' + INK + '"/><ellipse cx="20" cy="19.6" rx="1.8" ry="2.4" fill="' + INK + '"/><circle cx="12.65" cy="18.65" r="0.7" fill="#fff"/><circle cx="20.65" cy="18.65" r="0.7" fill="#fff"/></g>' +
          stroke('M13.9 22.9 Q16 24.8 18.1 22.9', INK, 1.5) +
          '<g fill="' + BLUSH + '" opacity="0.45"><ellipse cx="9.9" cy="22.3" rx="2" ry="1.15"/><ellipse cx="22.1" cy="22.3" rx="2" ry="1.15"/></g>'
        );
      case 'watch':
      case 'laptop':
        // Pupils live in .mk-eyes so the host can shift them toward the cursor.
        return (
          '<g class="mk-blink"><g class="mk-eyes"><ellipse cx="12" cy="19.4" rx="2.1" ry="2.7" fill="' + INK + '"/><ellipse cx="20" cy="19.4" rx="2.1" ry="2.7" fill="' + INK + '"/><circle cx="12.8" cy="18.3" r="0.85" fill="#fff"/><circle cx="20.8" cy="18.3" r="0.85" fill="#fff"/><circle cx="11.3" cy="20.4" r="0.45" fill="#fff" opacity="0.85"/><circle cx="19.3" cy="20.4" r="0.45" fill="#fff" opacity="0.85"/></g></g>' +
          '<ellipse cx="16" cy="23.3" rx="1.35" ry="1.6" fill="' + MOUTH + '"/>' +
          '<g fill="' + BLUSH + '" opacity="0.5"><ellipse cx="9.9" cy="22.3" rx="2" ry="1.15"/><ellipse cx="22.1" cy="22.3" rx="2" ry="1.15"/></g>'
        );
      case 'yawn':
        ...
        return (
          ...
          '<g fill="' + BLUSH + '" opacity="0.7"><ellipse cx="9.7" cy="22" rx="2.1" ry="1.2"/><ellipse cx="22.3" cy="22" rx="2.1" ry="1.2"/></g>'
        );
      case 'heavy':
        return (
      ...
      case 'nap':
        ...
        return (
          ...
          '<g fill="' + BLUSH + '" opacity="0.7"><ellipse cx="9.7" cy="22.5" rx="2.1" ry="1.2"/><ellipse cx="22.3" cy="22.5" rx="2.1" ry="1.2"/></g>'
        );
    }
    return '';
  }
  ...
  function head(pose) {
    var pale = pose === 'critical';
    var skin = pale ? SKIN_PALE : SKIN;
    var ahoge = pale
  ...
  // ---- standing rigs (fresh / healthy / heavy) -----------------------------
  function standSvg(pose, size) {
    var cls = pose === 'fresh' ? 'sp-run' : pose === 'heavy' ? 'sp-trudge' : 'sp-walk';
    var lean = pose === 'heavy' ? 13 : pose === 'fresh' ? 9 : 4;
    var w = Math.round((size * 48) / 50);
    return (
  ...
  // ---- NEW: greeting wave (one arm up, happy) ------------------------------
  function waveSvg(size) {
    ...
    return (
      ...
      '<g class="sp-headG"><g transform="translate(12.4 0.5) scale(0.72)">' + head('fresh') + '</g></g>' +
      '</g></svg>'
    );
  }

  // ---- NEW: yawn + stretch (seated, both arms reaching up) -----------------
  function yawnSvg(size) {
    var w = Math.round((size * 48) / 50);
    return (
  ...
  // ---- NEW: doodling (seated, notepad on lap, pencil scribbling) -----------
  function doodleSvg(size) {
    var w = Math.round((size * 48) / 50);
    return (
      ...
      stroke('M20 31.7 L27.4 31', INK, 0.6) + stroke('M20.4 33.2 L27.8 32.5', INK, 0.6) + '</g>' +
      // resting left hand
      stroke('M20.4 25.8 C19.1 28.6 19.6 31 21.4 32', LIMB, 2.3) +
      // pencil hand, scribbling
  ...
  function lyingSvg(size) {
    var w = Math.round((size * 64) / 50);
    return (
  ...
  // Shared seated lower body: hips ~y34, legs dangle straight down and swing
  // from the hip. Feet rest just over the pill's top edge.
  function dangleLegs() {
    return (
  ...
  var SEATED_DRESS = '<path d="M20.4 24 L27.6 24 C29.6 28.2 30.6 31.3 31.2 35.6 L16.8 35.6 C17.4 31.3 18.4 28.2 20.4 24 Z" fill="' + DRESS + '"/>';

  // Sit: perched on the edge, hands resting on the lap, feet swinging.
  function sitSvg(size) {
    var w = Math.round((size * 48) / 50);
    return (
      ...
      '<g class="sp-headG"><g transform="translate(12.4 3.4) scale(0.72)">' + head('fresh') + '</g></g>' +
      '</g></svg>'
    );
  }

  // Laptop: seated, a tiny laptop open on her lap, hands tapping the deck,
  // head tipped down into the glow.
  function laptopSvg(size) {
    var w = Math.round((size * 48) / 50);
    return (
      ...
      // laptop on the lap (drawn over the dress, under the hands)
      '<g class="sp-lap">' +
  ...
  // Nap: slumped to one side, head lolled, slow breathing. The "zzz" is drawn
  // by the host in the pill spot so it can rise past the sprite bounds.
  function napSvg(size) {
    var w = Math.round((size * 48) / 50);
    return (
  ...
  function spriteSvg(pose, size) {
    switch (pose) {
      case 'critical': return lyingSvg(size);
      case 'watch': return watchSvg(size);
      case 'cheer': return cheerSvg(size);
      case 'sit': return sitSvg(size);
      case 'laptop': return laptopSvg(size);
      case 'nap': return napSvg(size);
      case 'wave': return waveSvg(size);
      case 'yawn': return yawnSvg(size);
      case 'doodle': return doodleSvg(size);
      default: return standSvg(pose, size);
    }
  }
  ...
  window.Memoko = {
    ...
    spriteSvg: spriteSvg,
    STATUS: STATUS,
    POSES: ['fresh', 'healthy', 'heavy', 'critical', 'watch', 'cheer', 'sit', 'laptop', 'nap', 'wave', 'yawn', 'doodle'],
  };
})();
